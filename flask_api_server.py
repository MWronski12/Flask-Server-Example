...
LED_G_GPIO_PIN = 20
LED_B_GPIO_PIN = 21
led_r = None # PWMLED Instance. See init_led()
led_g = None
led_b = None
state = {                                                                            # (6)
    'red': 50, # % brightless of LED.
    'green': 50,
    'blue': 50
}
...
def init_led():
    ...
    global led_g
    global led_b
    led_r = PWMLED(LED_R_GPIO_PIN)
    led_g = PWMLED(LED_G_GPIO_PIN)
    led_b = PWMLED(LED_B_GPIO_PIN)
    led_r.value = state['red'] / 100                                                 # (7)
    led_g.value = state['green'] / 100
    led_b.value = state['blue'] / 100
...
def index():
    ...
    return render_template('index.html', pin=LED_PIN)                                # (9)
...
class LEDControl(Resource):                                                          # (10)
    ...
    def __init__(self):
        ...
        self.args_parser.add_argument(
            name='red',  # Name of arguement
            required=False,  # Mandatory arguement
            type=inputs.int_range(0, 100),  # Allowed range 0..100                   # (12)
            help='Set red LED brightness level {error_msg}',
            default=None)
        self.args_parser.add_argument(
            name='green',  # Name of arguement
            required=False,  # Mandatory arguement
            type=inputs.int_range(0, 100),  # Allowed range 0..100
            help='Set green LED brightness level {error_msg}',
            default=None)
        self.args_parser.add_argument(
            name='blue',  # Name of arguement
            required=False,  # Mandatory arguement
            type=inputs.int_range(0, 100),  # Allowed range 0..100
            help='Set blue LED brightness level {error_msg}',
            default=None)
    ...
    def post(self):
        ...
        if args.red:
            state['red'] = args.red                                                      # (16)
            led_r.value = state['red'] / 100                                             # (17)
            logger.info("Red LED brightness level is " + str(state['red']))
        if args.green:
            state['green'] = args.green
            led_g.value = state['green'] / 100
            logger.info("Green LED brightness level is " + str(state['green']))
        if args.blue:
            state['blue'] = args.blue
            led_b.value = state['blue'] / 100
            logger.info("Blue LED brightness level is " + str(state['blue']))
